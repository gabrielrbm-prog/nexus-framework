...
class NexusCodeAgentV4 {
    ...
    if (fs.existsSync(manifestPath)) {
      try {
        ...
        (manifest.images || []).forEach(img => {
          ...
          this._images[cat].push('../assets/images/' + path.basename(img.path));
        });
    ...
    if (fs.existsSync(videoManifestPath)) {
      try {
        ...
        this._videos = (vManifest.videos || []).map(v => ({
          ...v,
          path: '../assets/videos/' + path.basename(v.path || '')
        }));
        console.log('  [videos] Loaded manifest:', this._videos.length + ' videos');
      } catch(e) { /* silent */ }
    }
  }
  ...
  buildPage() {
    ...
    const heroHasImage = this._images && this._images.hero && this._images.hero.length > 0;
    const heroImageTag = heroHasImage
      ? `<img src="${this._images.hero[0]}" alt="" class="hero-bg-image" loading="eager">\n      <div class="hero-overlay"></div>`
      : '';
    ...
    const videoUrl = this._videos.length > 0 ? this._videos[0].path : '';
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hero-overlay { position: absolute; inset: 0; background: linear-gradient(to bottom, rgba(0,0,0,0.6), rgba(0,0,0,0.8)); z-index: 1; }
